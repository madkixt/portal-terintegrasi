...
class TextHelper {
	...
	public function maxAllRowsLength($conn, $query, $dbms) {
		$lengths = array();
		$stop = false;
		$offset = 0;
		$first = true;
		...
		while (!$stop) {
			$data = SiteController::query($conn, $query, $dbms, SiteController::MAX_ROWS, $offset);
			if (count($data) === 0)
				break;
				
			if (count($data) < SiteController::MAX_ROWS)
				$stop = true;
			
			if ($first) {
				$first = false;
				$lengths = $this->maxLengths($data);
				continue;
			}
			
			$pLength = $this->maxLengths($data);
			foreach ($pLength as $key => $value) {
				if ($value > $lengths[$key])
					$lengths[$key] = $value;
			}
			
			$offset += SiteController::MAX_ROWS;
		}
